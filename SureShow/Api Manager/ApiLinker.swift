...
class ApiLinker: NSObject {
    ...
    func execute(url:String,image:UIImage,withName:String,parameters: @escaping () -> [String:AnyObject],onResponse:@escaping (_ response:AnyObject?)->Void,onError:@escaping (_ error:String?)->Void){
        ...
        DispatchQueue.global(qos: .background).async {
            self.postRequestMultipart(withName: withName,image: image, parameters: parameters(), responseD: { (resp) in
                onResponse(resp)
                if self.isProgress {
                    self.hideProgress()
                }
...
//    }
    ...
    fileprivate func get(response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        AF.request(url ,headers: headers).responseString{ response in }.responseJSON{ respons in
            
            
            if (respons.error == nil){
                let responseObject = respons.value
                response(responseObject as AnyObject)
                
            }else{
                error(respons.error.debugDescription)
                
            }
        }
    }
    fileprivate func stringGet(response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        AF.request(url ,headers: headers).responseString{resposne  in
            if (resposne.error == nil){
                let responseObject = resposne.value
                response(responseObject as AnyObject)
                
            }else{
                error(resposne.error.debugDescription)
    ...
    fileprivate func put(params:[String:AnyObject]?,response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        ...
        AF.request(url,method:.put,parameters: parameters,/*encoding: JSONEncoding.default,*/headers:getHeaders()).responseString{response in}
            .responseJSON{ resp in
                
                if (resp.error == nil){
                    let responseObject = resp.value
                    
                    response(responseObject as AnyObject)
                }else{
                    error(resp.error.debugDescription)
                }
        }
    ...
    fileprivate func post(params:[String:AnyObject]?,response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        ...
        AF.request(url, method: .post, parameters: parameters, headers: getHeaders())
    ...
    fileprivate func stringPost(params:[String:AnyObject]?,response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        ...
        AF.request(url,method:.post,parameters: parameters,/*encoding: JSONEncoding.default,*/headers:getHeaders()).responseString{responses in
            
            
            if (responses.error == nil){
                let responseObject = responses.value
                
                response(responseObject as AnyObject)
            }else{
                error(responses.error.debugDescription)
            }
        }
    ...
    fileprivate func httpPost(params:[String:AnyObject]?,response:@escaping (_ response:AnyObject?)->Void,error:@escaping (_ error:String?)->Void){
        ...
        AF.request(url, method: .post, parameters: parameters, encoding: JSONEncoding(options:[]), headers: headers).responseJSON { (resp) in
     ...
     */
    fileprivate func   postRequestMultipart(withName:String,image:UIImage!,parameters:[String:AnyObject],responseD: @escaping (_ response:AnyObject)->Void,error:@escaping (_ error:String)->Void){
...
//        }
    ...
    func download(downloadUrl:String,fileName:String,dirName:String, completion: ((Bool, String?) -> Void)?){
        ...
        AF.download(downloadUrl, method: .post, parameters: nil, encoding: JSONEncoding.default, headers: nil, to:destination ).downloadProgress { (progress) in
            print(progress)
            }.responseString { (response) in
                if self.isProgress{
                    self.hideProgress()
    ...
    func setHeader(headers:HTTPHeaders) -> ApiLinker{
